...
class SetupForm : Form
{
    ...
    Panel CreateSideBar()
    {
        ...
        p.Paint += (s, e) => {
            ...
            using (var verFont = new Font("Segoe UI", 10))
                e.Graphics.DrawString("v2026.06.12", verFont, new SolidBrush(Color.FromArgb(180, 255, 255, 255)), new RectangleF(0, 260, p.Width, 22), sf);
        };
        return p;
    }
    ...
    void BuildDone()
    {
        ...
        var info = new Label {
            Text = "游泳赛事管理与计时系统 v2026.06.12 已成功安装。\n\n" +
                   "已创建桌面快捷方式：\n" +
                   "  ★  游泳赛事管理主服务器\n" +
                   "  ★  远程计时控制台\n" +
                   "  ★  远程显示控制台\n" +
